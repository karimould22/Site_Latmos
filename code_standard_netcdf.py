...
def charger_donnees(fichiers):
    ...
    try:
        ...
        # EXTRACTION DYNAMIQUE DES METADONNEES (Unites et Descriptions)
        unites = {}
        descriptions = {}
        ...
        # EXTRACTION DYNAMIQUE DE LA STATION ET DU LIEU
        # On essaie d'abord de lire les attributs globaux du NetCDF
        ...
        # S'ils sont vides, on essaie de deviner a partir du nom du premier fichier
        if not nom_station or not nom_lieu:
            ...
            if not nom_station and len(parties_nom) > 1:
                nom_station = parties_nom[1] # Ex: QUALAIR_GMX500 -> GMX500
            if not nom_lieu and len(parties_nom) > 2:
                nom_lieu = parties_nom[2]    # Ex: QUALAIR_GMX500_terrasse -> terrasse
...
# On charge les donnees et on recupere la station et le lieu dynamiquement
...
if df is not None and not df.empty:
    ...
    # --- INFORMATIONS ENTIEREMENT DYNAMIQUES ---
    st.sidebar.markdown("---")
    ...
    if len(dates_selectionnees) == 2:
        ...
        st.dataframe(stats.to_frame(name="Valeurs").T, use_container_width=True)
