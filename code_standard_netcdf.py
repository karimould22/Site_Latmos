...
if df is not None and not df.empty:
    ...
    st.sidebar.info(
        ...
        f"Lieu : {lieu_dynamique}\n\n"
        f"Fichiers charges : {len(fichiers_upload)}\n\n"
        f"Dates dispo : {df.index.min().strftime('%d/%m/%Y')} au {df.index.max().strftime('%d/%m/%Y')}"
    )
    ...
    if len(dates_selectionnees) == 2:
        ...
        st.markdown("---")
        st.markdown(f"""
            <p style='color: white; font-size: 22px; font-weight: bold; text-shadow: none !important; margin-bottom: 0px;'>
        ...
        stats_propres = stats.drop(['25%', '50%', '75%']) # Suppression des colonnes inutiles
        st.dataframe(stats_propres.to_frame(name="Valeurs").T, use_container_width=True)
        
        # --- NOUVEAU : SYNTHESES ANNUELLES ET SAISONNIERES ---
        st.markdown(f"""
            <p style='color: white; font-size: 20px; font-weight: bold; text-shadow: none !important; margin-top: 20px;'>
            Syntheses annuelles et saisonnieres :
            </p>
            """, unsafe_allow_html=True)
            
        # Creation de deux colonnes pour afficher les tableaux cote a cote
        col1, col2 = st.columns(2)
        
        with col1:
            st.markdown("<p style='color: white; font-weight: bold; text-shadow: none !important;'>Par Annee</p>", unsafe_allow_html=True)
            # Grouper par annee
        ...
        with col2:
            st.markdown("<p style='color: white; font-weight: bold; text-shadow: none !important;'>Par Saison</p>", unsafe_allow_html=True)
            # Definition des saisons (Meteorologiques)
            saisons_dict = {
            ...
            saison_series = df_filtre.index.month.map(saisons_dict)
            df_saison = df_filtre[variable_choisie].groupby(saison_series).agg(['mean', 'min', 'max'])
            df_saison.index.name = "Saison"
            df_saison.columns = ["Moyenne", "Min", "Max"]
            
            # Reorganiser l'ordre d'affichage pour que ce soit logique
            ordre_saisons = ["Hiver", "Printemps", "Ete", "Automne"]
            df_saison = df_saison.reindex([s for s in ordre_saisons if s in df_saison.index])
