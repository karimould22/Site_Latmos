...
# --- CHARGEMENT DYNAMIQUE DES DONNEES (AVEC SECURITE ANTI-CRASH) ---
@st.cache_data
def charger_donnees(fichiers):
    ...
    try:
        ...
        # DETECTION INTELLIGENTE DU TEMPS
        colonne_temps = None
        mots_cles_temps = ['time', 'date', 'datetime', 'time_counter', 't', 'temps']
        
        for col in df.columns:
            if str(col).lower() in mots_cles_temps:
                colonne_temps = col
                break
                
        if colonne_temps:
            df[colonne_temps] = pd.to_datetime(df[colonne_temps])
            df = df.set_index(colonne_temps)
        else:
            st.error("Impossible de trouver une dimension de temps (time, date...) dans ce fichier NetCDF. Verifiez la structure du fichier.")
            return None, None, None, None, None
            
        # SECURITE : Verifier que l'index est bien temporel
        if not isinstance(df.index, pd.DatetimeIndex):
            st.error("L'index temporel n'a pas pu etre cree correctement. Ce fichier contient peut-etre des dimensions complexes (ex: lat/lon sans reduction temporelle).")
            return None, None, None, None, None
        ...
        st.error("Le fichier est trop lourd ou contient trop de dimensions. L'application manque de memoire pour l'afficher en entier.")
        return None, None, None, None, None
    except Exception as e:
...
# --- SI DES FICHIERS SONT CHARGES ET VALIDES ---
...
elif fichiers_upload:
    # Si des fichiers sont charges mais que df est vide/None, l'erreur a deja ete affichee par la fonction
    pass
else:
